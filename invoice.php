...
try {
  ...
  $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    $stmt = $conn->prepare("SELECT * FROM tbl_orders_a180900, tbl_staffs_a180900_mypt2,
      tbl_customers_a180900_mypt2, tbl_orders_details_a180900 WHERE
      tbl_orders_a180900.fld_staff_num = tbl_staffs_a180900_mypt2.FLD_STAFF_ID AND
      tbl_orders_a180900.fld_customer_num = tbl_customers_a180900_mypt2.FLD_CUSTOMER_ID AND
      tbl_orders_a180900.fld_order_num = tbl_orders_details_a180900.fld_order_num AND
      tbl_orders_a180900.fld_order_num = :oid");
...
<table class="table table-bordered">
  ...
  try {
    $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
      $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a180900,
        tbl_products_a180900_mypt2 where 
        tbl_orders_details_a180900.fld_product_num = tbl_products_a180900_mypt2.FLD_PRODUCT_ID AND
        fld_order_num = :oid");
    $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
      $oid = $_GET['oid'];
    $stmt->execute();
